...
public class ReserveService {
	...
	*/
	...
	public List<Reserve> findByDateAndSeat(LocalDate date, Integer seat) {
		return reserveRepository.findByReserveDateAndSeatId(date, seat);
	}

	//	指定された日付とユーザーに基づいて予約情報を取得するメソッド
	//	    ConfirmationControllerで重複確認するのに使用
	public List<Reserve> findByUser(Integer userId) {
		return reserveRepository.findByUserId(userId);
	}

	//		指定された日付と座席に基づいて予約情報を取得するメソッド
	//	    削除機能なのか予約機能なのかの分岐に使用
	public List<Reserve> deleteByReserveId(Integer reserveId) {
		return reserveRepository.deleteByReserveId(reserveId);
	}
